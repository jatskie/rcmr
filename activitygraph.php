...
$graph->y_format['logins']        = array(
	 'colour' => 'red',
	'line' => 'line',
	'legend' => 'User Logins' 
);
$graph->y_format['course_views']  = array(
	 'colour' => 'blue',
	'line' => 'line',
	'legend' => 'Course Activity' 
);
$graph->y_format['quiz']          = array(
	 'colour' => 'gray',
	'line' => 'line',
	'legend' => 'Quiz Activity' 
);
$graph->y_format['feedback']      = array(
	 'colour' => 'green',
	'line' => 'line',
	'legend' => 'Feedback' 
);
...
for ( $i = 0; $i <= 16; $i++ )
{
	...
	$query           = sprintf( "SELECT COUNT(DISTINCT userid) AS no_logins  FROM " . $CFG->prefix . "log WHERE time > %d AND time < %d and module = 'user' and action ='login'", $timestamp, $end_timestamp );
	$no_logins       = 0;
	...
	$query   = sprintf( "SELECT COUNT(id) AS course_views FROM " . $CFG->prefix . "log WHERE time > %d AND time < %d AND module = 'course'", $timestamp, $end_timestamp );
	$c_views = 0;
	...
	$query          = sprintf( "SELECT COUNT(DISTINCT id) AS feedback FROM " . $CFG->prefix . "log WHERE time > %d AND time < %d and module = 'feedback'", $timestamp, $end_timestamp );
	$feedback_stats = 0;
	...
	$query      = sprintf( "SELECT COUNT(DISTINCT(id)) AS quiz_activity FROM " . $CFG->prefix . "log WHERE time > %d AND time < %d and module = 'quiz' AND action IN ('view', 'attempt', 'continue attemp', 'close attempt', 'report', 'view all', 'review')", $timestamp, $end_timestamp );
	$quiz_stats = 0;
